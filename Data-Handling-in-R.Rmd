...
以上参数中需要注意：detecDates参数，当你的Excel表格中带日期列时需要将参数设置为TRUE,不然将会把日期识别为数字读入。
...
数据清洗完后，或者是分组聚合完成，需要将结果从R导出到Excle，这时就利用`write.xlsx()`函数将结果数据集写入Excel中。
...
tidyxl:Imports non-tabular from Excel files into R。tidyxl将不规则数据导入R。
我们可以通过`readxl`包读取整洁的excel数据，那如果是Excel的透视表怎么读取？
