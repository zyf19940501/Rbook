...
由于能力有限以及处理的日期数据类型有限，本章节仅就常规问题解决提供一种解法。
...
`lubridate`包是对Base R中POSIXct类的封装。所以无论从函数名还是功能等方面，lubridate包中的函数功能更加清晰明了。获取当前日期、时间；拆解时间日期中的年、月、日、星期、计算年月天数，记录时刻等常用的时间日期功能，`lubridate`包中都有相对应的功能函数。

本人在处理时间时常用R包`lubridate`，本节将介绍包中部分函数用法。



__Note:lubridate默认使用UTC时区__
