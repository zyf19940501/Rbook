...
作为商业数据分析师，我学习使用\texttt{R}以及有一段时间，主要用\texttt{R}来处理数据，自动化报表，可视化等，\texttt{shiny}等。\texttt{R}极大的拓展了数据处理能力,让我有更多精力聚焦在具体的业务问题上，所以很感谢\texttt{R}语言。

本文主要记录个人学习过程中的经验，因个人能力有限，难免出现错误，如阅读中发现错误，欢迎联系本人更正。

联系邮箱:\href{mailto:598253220@qq.com}{\nolinkurl{598253220@qq.com}}
...
\CommentTok{\#\textgreater{}  [1] compiler\_4.0.3    magrittr\_2.0.1    bookdown\_0.21     htmltools\_0.5.1.1}
...
\CommentTok{\#\textgreater{} [13] xfun\_0.21         rlang\_0.4.10      evaluate\_0.14}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\textgreater{} Warning: 程辑包\textquotesingle{}data.table\textquotesingle{}是用R版本4.0.4 来建造的}
...
\section{i j by 使用}\label{i-j-by-ux4f7fux7528}}
...
\hypertarget{ux5217ux7b5bux9009}{%
\subsection{列筛选}\label{ux5217ux7b5bux9009}}
...
\hypertarget{j-ux5e38ux89c4ux8ba1ux7b97}{%
\subsection{j 常规计算}\label{j-ux5e38ux89c4ux8ba1ux7b97}}
...
\subsection{by 分组}\label{by-ux5206ux7ec4}}
...
\hypertarget{ux884cux7b5bux9009-1}{%
\subsection{行筛选}\label{ux884cux7b5bux9009-1}}
...
\FunctionTok{wget}\NormalTok{ {-}qO{-} https://packages.microsoft.com/keys/microsoft.asc }\KeywordTok{|} \FunctionTok{sudo}\NormalTok{ apt{-}key add {-}}

\CommentTok{\# 为 SQL Server 2019 注册 Microsoft SQL Server Ubuntu 存储库}
\FunctionTok{sudo}\NormalTok{ add{-}apt{-}repository }\StringTok{"}\VariableTok{$(}\FunctionTok{wget}\NormalTok{ {-}qO{-} https://packages.microsoft.com/config/ubuntu/20.04/mssql{-}server{-}2019.list}\VariableTok{)}\StringTok{"}
...
\FunctionTok{sudo}\NormalTok{ apt{-}get install {-}y mssql{-}server}

\CommentTok{\# 验证服务是否运行}
\ExtensionTok{systemctl}\NormalTok{ status mssql{-}server {-}{-}no{-}pager}
\end{Highlighting}
\end{Shaded}

至于其他如安装sql server 命令行工具请{[}查阅官网{]}自行安装(\url{https://docs.microsoft.com/zh-cn/sql/linux/quickstart-install-connect-ubuntu?view=sql-server-linux-ver15\&preserve-view=true})。
...
如果你是用windows系统，自己安装数据库后，并配置好账户密码，启动了数据库。发现乱码时，根据数据库编码指定\texttt{encoding}参数即可，常规在win下面用sql server设置\texttt{encoding\ =\ "GBK"}。
...
\texttt{MySQL()}函数来源\texttt{RMySQL}包，用来创建\texttt{\textless{}MySQLDriver\textgreater{}}驱动
...
\NormalTok{)}
\CommentTok{\#mysql 数据库默认端口是3306 访问不通时记得检查3306端口是否开放}
\end{Highlighting}
\end{Shaded}

\hypertarget{ux67e5ux8be2ux4e0aux4f20ux7b49ux4efbux52a1}{%
\subsection{查询上传等任务}\label{ux67e5ux8be2ux4e0aux4f20ux7b49ux4efbux52a1}}
...
如果是linux上通过odbc连接Sql Server,在连接字符中设置字符编码charset=zh\_CN.GBK，如果设置为gbk会报错。
...
在第一次安装这个包时遇到了很多困难，首先需要安装oracle客户端，其次配置好环境变量，最后安装包。应该与Oracle的连接需要安装\href{https://www.oracle.com/database/technologies/instant-client.html}{Oracle Instant Client}，
...
\href{https://mp.weixin.qq.com/s?src=11\&timestamp=1615369254\&ver=2938\&signature=5EYogLq8uU76XutqyrSggmI8mzylgzPNtKFix*WGTyxSLmbiX3HvNhCXajeGkqDJNh06WE4UPveuz5WxibQeR2usoXyWPo-Wq7maE6DbSMzub8yadc0oXDiau-D1bKLT\&new=1}{公众号文章连接}
...
\texttt{RMySQL}包的主要作用可以提供驱动与mysql数据库进行连接，在本机未安装mysql的驱动的情况下
...
\CommentTok{\#dbListTables(con) 查看该库全部表格}
\end{Highlighting}
\end{Shaded}
...
Excle中自带的字符函数如: \texttt{left},\texttt{len},\texttt{mid},\texttt{find},\texttt{Proper},\texttt{rept},\texttt{trim},\texttt{upper},\texttt{substitute},\texttt{concatenate},以及\texttt{Excle}2019新出的\texttt{concat},`TEX

TJOIN\texttt{等函数，新出的}textjoin`函数我个人比较喜欢用。
...
R中日期 : 2021-04-25 转化成数字: 18742,
Excel 中 2021-04-25转化成数字:44310,两者相差25568.
...
\toprule
col1 & col2 & col3 & col4 & col5 & col6 & col7\tabularnewline
\midrule
\endhead
v1 & v2 & v3 & v4 & v5 & v6 & v7\tabularnewline
vb1 & vb2 & vb3 & vb4 & vb5 & vb6 & vb7\tabularnewline
\bottomrule
\end{longtable}
